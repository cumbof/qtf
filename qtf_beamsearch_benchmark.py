...
def residue_assignments(dof_entries: List[Dict[str, Any]],
                        ...
                        chi_rotamers: List[float]) -> List[np.ndarray]:
    """
    ...
    if chi_types:
        # for each backbone assignment, expand chi rotamers by cartesian product
        assigns = []
        for b in backbone_assigns:
            # each chi dof independent 3-state
            for chi_vals in np.array(np.meshgrid(*([chi_rotamers] * len(chi_types)))).T.reshape(-1, len(chi_types)):
                d = dict(b)
                for t, v in zip(chi_types, chi_vals):
                    d[t] = float(v)
                assigns.append(d)
    else:
...
def main():
    ...
    basin_centers = [(-60, -45), (-135, 135), (-75, 145), (-60, 120), (60, 45)]
    ...
    for r in range(L):
        residue_opts[r] = residue_assignments(dofs_by_res.get(r, []), backbone_pairs, chi_rot)
