...
def test_builtin_recipes_load():
    ...
    assert "engine" not in recipes["qtf-three-stage-main"]
    assert recipes["qtf-three-stage-main"]["phases"][0]["score_model"] == "pheat-physics"
    assert recipes["qtf-heavy-atom-phased"]["phases"][0]["score_model"].startswith("pheat-")
    assert recipes["qtf-three-stage-main"]["circuit_template"]["name"] == "EfficientSU2"
    assert recipes["qtf-heavy-atom-phased"]["circuit_template"]["source"] == "qtf"
    assert recipes["qtf-three-stage-main"]["geometry"]["stored_angles"] == []
    assert recipes["qtf-three-stage-main"]["geometry"]["stored_lengths"] == []
    assert "chi_mode" not in recipes["qtf-three-stage-main"]["geometry"]
