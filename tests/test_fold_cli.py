...
def test_builtin_recipes_load():
    ...
    assert "engine" not in recipes["qtf-three-phase"]
    assert recipes["qtf-three-phase"]["phases"][0]["score_model"] == "amber"
    assert recipes["qtf-heavy-atom-phased"]["phases"][0]["score_model"].startswith("pheat-")
    assert recipes["qtf-three-phase"]["circuit_template"]["name"] == "EfficientSU2"
    assert recipes["qtf-heavy-atom-phased"]["circuit_template"]["source"] == "qtf"
    assert recipes["qtf-three-phase"]["geometry"]["stored_angles"] == []
    assert recipes["qtf-three-phase"]["geometry"]["stored_lengths"] == []
    assert "chi_mode" not in recipes["qtf-three-phase"]["geometry"]
...
def test_command_line_redacts_ibm_token_and_crn():
    ...
    override = qtf_engine._command_line(
        None,
        "qtf fold --ibm-token direct-secret-token --ibm-crn crn:v1:secret-instance",
    )
...
def test_fold_cli_passes_length_encoding_options_from_recipe():
    ...
    recipe = {
        ...
        "phases": [{"name": "phase-a", "optimizer": "COBYLA", "score_model": "amber"}],
    }
