...
class QuantumBiophysicsFolder:
    ...
    def _get_angles(self, params, mode: str = "statevector", backend=None, shots: int = 4096):
        ...
        if mode == "sampler":
            ...
            def _basis_circuit(qc, basis: str):
                """Return a copy of qc with basis-change gates + measure_all."""
                c = qc.copy()
                if basis == "X":
                    for q in range(n):
                        c.h(q)
            ...
            def _expect(counts):
                """Single-qubit and nearest-neighbour expectations from counts."""
                ...
                for bitstring, c in counts.items():
                    ...
                    if len(bs) < n:
                        bs = bs + "0" * (n - len(bs))
                    zv     = np.array([_bit(bs[i]) for i in range(n)], dtype=float)
                    single += p * zv
                    if n >= 2:
                        pair += p * (zv[:-1] * zv[1:])
                if total > 0:
            ...
            # ── run Z / X / Y ─────────────────────────────────────────────────
            Z,  ZZ = _expect(_run(_basis_circuit(bound_circuit, "Z")))
            X,  XX = _expect(_run(_basis_circuit(bound_circuit, "X")))
            Y,  YY = _expect(_run(_basis_circuit(bound_circuit, "Y")))

            # ── Bloch-sphere decoding ──────────────────────────────────────────
            # phi_i  = atan2(Y_i, X_i)        → in [-π, π]
            # theta_i = arccos(Z_i)            → in [0, π], centred to [-π/2, π/2]
            phi   = np.arctan2(Y, X)
            theta = np.arccos(np.clip(Z, -1.0, 1.0)) - np.pi / 2.0

            angles_list = list(phi) + list(theta)           # 2n angles

            # correlation-based "psi-like" angles
            if n >= 2:
                corr      = 0.5 * ZZ + 0.25 * XX + 0.25 * YY
                psi_like  = np.arccos(np.clip(corr, -1.0, 1.0)) - np.pi / 2.0
                angles_list.extend(list(psi_like))           # n-1 more angles

            base = np.array(angles_list, dtype=float)

            # ── deterministic expansion to total_angles ────────────────────────
            if len(base) == 0:
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # FOLD — 3-stage classical optimisation, then hardware angle extraction
    # ──────────────────────────────────────────────────────────────────────────
    def fold(self, max_iter=2000, initial_params=None,
        ...
        Parameters
        ----------
        max_iter        : int    — max iterations per optimiser stage
        initial_params  : array  — optional starting params
        hw_backend      : Qiskit backend or None
            None        → final angles from Statevector (pure classical)
            AerSimulator() → shot-based simulation (test before hardware)
            real IBM backend → actual hardware execution
        hw_shots        : int    — shots for hardware angle extraction
        """
