...
class QuantumBiophysicsFolder:
    ...
    def _get_angles(self, params, backend=None, shots=4096):
        """
        Hardware-compatible torsion extraction:
        (measurement counts) -> (expectations/correlations) -> (torsion angles)

        Key ideas:
        - Measure in multiple bases (Z, X, Y) to get richer features without statevector phases.
        - Use sparse pair correlations (nearest neighbors) to add more signal.
        - Decode per-qubit angles using Bloch-sphere style mapping:
            phi_i = atan2(<Y_i>, <X_i>)
            theta_i = arccos(clip(<Z_i>))
        then use correlations to create extra "psi-like" angles.
        - Finally, deterministically expand/compress to self.total_angles.
        """
        ...
        # ----------------------------
        if backend is None:
            backend = AerSimulator()  # local simulator; replace with IBM backend for real hardware

        # ----------------------------
        # 1) Bind parameters to ansatz
        # ----------------------------
        param_dict = dict(zip(self.ansatz.parameters, params))
        base_qc = self.ansatz.assign_parameters(param_dict)

        n = self.n_qubits  # number of system qubits in ansatz

        # ----------------------------
        # Helpers
        # ----------------------------
        def _measured_in_basis(qc, basis: str):
            """
            Return a NEW circuit that measures all qubits in the given basis.
            basis: 'Z', 'X', or 'Y'
            """
            c = qc.copy()

            # Ensure classical bits exist (measure_all adds them if needed)
            # Apply basis change before measuring:
            if basis.upper() == "X":
                # Measure X by H then measure Z
                for q in range(n):
                    c.h(q)
            elif basis.upper() == "Y":
                # Measure Y by Sdg then H then measure Z
                for q in range(n):
                    c.sdg(q)
                    c.h(q)
            elif basis.upper() == "Z":
                ...
                raise ValueError("basis must be one of {'Z','X','Y'}")
        ...
        def _bit_to_pm1(bit_char: str) -> int:
            # Qiskit: '0' -> +1 eigenvalue, '1' -> -1 eigenvalue (Pauli Z convention)
            return +1 if bit_char == "0" else -1

        def _expectations_from_counts(counts):
            """
            From counts over n bits, compute:
            - single expectations: <b_i> where b_i ∈ {+1,-1}
            - nearest-neighbor correlations: <b_i b_{i+1}>

            Important: Qiskit count keys are returned as strings with classical bit order.
            When using measure_all(), the rightmost character typically corresponds to qubit 0.
            We reverse the string so index i aligns with qubit i.
            """
            total = 0
            single = np.zeros(n, dtype=float)
            pair = np.zeros(max(n - 1, 0), dtype=float)

            for bitstring, c in counts.items():
                p = c / shots
                total += p

                bs = bitstring.replace(" ", "")[::-1]  # reverse so bs[i] matches qubit i
                # Safety if some backends include fewer bits (rare), pad with zeros:
                if len(bs) < n:
                    bs = bs + ("0" * (n - len(bs)))

                zvals = np.array([_bit_to_pm1(bs[i]) for i in range(n)], dtype=float)

                single += p * zvals
                if n >= 2:
                    pair += p * (zvals[:-1] * zvals[1:])
        ...
        # ----------------------------
        # 2) Run circuits in Z/X/Y bases
        # ----------------------------
        counts_Z = _run_counts(_measured_in_basis(base_qc, "Z"))
        counts_X = _run_counts(_measured_in_basis(base_qc, "X"))
        counts_Y = _run_counts(_measured_in_basis(base_qc, "Y"))

        Z, ZZ = _expectations_from_counts(counts_Z)  # Z_i and Z_i Z_{i+1}
        X, XX = _expectations_from_counts(counts_X)  # X_i and X_i X_{i+1} (after basis map)
        Y, YY = _expectations_from_counts(counts_Y)  # Y_i and Y_i Y_{i+1}

        # ----------------------------
        # 3) Decode "primary" angles per qubit from Bloch components
        # ----------------------------
        # phi_i from atan2(Y, X) in [-pi, pi]
        phi = np.arctan2(Y, X)

        # theta_i from arccos(Z) in [0, pi]
        Zc = np.clip(Z, -1.0, 1.0)
        theta = np.arccos(Zc)

        # ----------------------------
        # 4) Make extra "psi-like" angles from correlations
        #    (Use a stable mapping: arccos(correlation) in [0, pi], then shift to [-pi, pi] if desired)
        # ----------------------------
        angles_list = []
        angles_list.extend(phi.tolist())        # n angles
        angles_list.extend((theta - np.pi/2).tolist())  # n angles centered around 0

        # Correlation-based angles
        if n >= 2:
            # Use Z-correlations primarily (like backbone coupling), plus a little XX/YY signal
            corr = 0.5 * ZZ + 0.25 * XX + 0.25 * YY
            corr = np.clip(corr, -1.0, 1.0)
            psi_like = np.arccos(corr) - np.pi/2  # center around 0
            angles_list.extend(psi_like.tolist())  # (n-1) angles

        base_angles = np.array(angles_list, dtype=float)

        # ----------------------------
        # 5) Expand / compress deterministically to self.total_angles
        #    IMPORTANT: base_angles length is O(n), but total_angles can be larger.
        #    We create additional angles via deterministic mixing (no randomness).
        # ----------------------------
        if len(base_angles) == 0:
            base_angles = np.zeros(1, dtype=float)

        if len(base_angles) >= self.total_angles:
            return base_angles[: self.total_angles]

        # Deterministic expansion using a simple sinusoidal mixing bank.
        # This keeps outputs bounded in [-pi, pi] and gives each torsion a distinct value.
        out = np.zeros(self.total_angles, dtype=float)
        out[: len(base_angles)] = base_angles

        # Fill the rest by mixing base angles in a structured way
        L = len(base_angles)
        for k in range(L, self.total_angles):
            ...
            v = (
                0.60 * base_angles[i]
                + 0.30 * np.sin(base_angles[j])
                + 0.10 * np.cos(base_angles[m])
            )
            # Wrap to [-pi, pi]
